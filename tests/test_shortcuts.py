...
def test_colors_import():
    from gitfast.utils.colors import Colors, Printer
...
def test_shell_detection():
    from gitfast.utils.os_utils import get_shell
    shell = get_shell()
    assert shell in ["bash", "zsh", "fish", "powershell"]


def test_shell_config_path():
    from gitfast.utils.os_utils import get_shell_config
...
def test_package_manager():
    from gitfast.utils.os_utils import get_package_manager
...
def test_git_not_repo():
    from gitfast.utils.git import is_git_repo
    tmp = tempfile.mkdtemp()
    os.chdir(tmp)
...
def test_current_branch():
    from gitfast.utils.git import current_branch
    tmp = setup_test_repo()
    os.chdir(tmp)
...
def test_get_status():
    from gitfast.utils.git import get_status
    tmp = setup_test_repo()
    os.chdir(tmp)
    status = get_status()
...
def test_stage_all():
    from gitfast.utils.git import stage_all, is_git_repo
    tmp = setup_test_repo()
    os.chdir(tmp)
...
def test_commit():
    from gitfast.utils.git import stage_all, commit
    tmp = setup_test_repo()
    os.chdir(tmp)
    run(f"git -C {tmp} config user.email 'test@test.com'")
    run(f"git -C {tmp} config user.name 'Test'")
